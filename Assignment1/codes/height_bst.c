...
int main(){
    //Using array given in question
    int in1[] = {8,6,9,4,7,2,5,1,3};
	int post1[] = {8,9,6,7,4,5,2,3,1};
    int n1 = sizeof(in1)/sizeof(in1[0]);

    struct Node* root1 = constructTree(in1, post1, n1);

    int ht1 = maxDepth(root1) - 1;
    printf("Height of Binary Tree is %d\n", ht1);

    //Verification by generating random binary tree
    int h ;
    printf("Enter required height: ");
    scanf("%d", &h);
    int x = 2*h+ 1;
    int *in2 = generate_order(h, 1);
    int *post2 = generate_order(h, 2);

    //Using generated inorder & postorder array to find height of binary tree
    struct Node* root2 = constructTree(in2, post2, x);
    int ht2 = maxDepth(root2) - 1;
    printf("\nHeight of Generated Binary Tree is %d\n", ht2);
    if(h == ht2){
        printf("Algorithm verified\n");
    }
    return 0;
}
